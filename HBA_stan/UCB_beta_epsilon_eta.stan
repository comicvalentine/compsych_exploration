...
parameters{
    ...
    array[nH] vector[4] mu_p; //[information bonus, inverse temperature, novelty bonus, value-free random]
    array[nH] vector<lower=0>[4] sigma;  //[information bonus, inverse temperature, novelty bonus, value-free random]
...
transformed parameters{
    ...
    for (h_idx in 1:nH){
        for (s_idx in 1:nS){
            ...
            eta[h_idx, s_idx] = Phi_approx(mu_p[h_idx][3] + sigma[h_idx][3]*eta_raw[h_idx, s_idx])*5; //boundary: (0, 5)
            epsilon[h_idx, s_idx] = Phi_approx(mu_p[h_idx][4] + sigma[h_idx][4]*epsilon_raw[h_idx, s_idx])*0.5; //boundary: (0, 0.5) 
    ...
    for (h_idx in 1:nH) {
        ...
        for (s_idx in 1:nS) {
            ...
            for (bt_idx in 1:nBT) {
                ...
                V_n = Q_n + gamma[h_idx, s_idx] .* sigma_n + eta[h_idx, s_idx] .* novelty[h_idx, s_idx, bt_idx];
